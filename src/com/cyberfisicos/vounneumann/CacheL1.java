package com.cyberfisicos.vounneumann;
...
public class CacheL1 extends Memoria{
    ...
    public int read(int pos) throws EnderecoInvalido {
        ...
        if (isNotOnCache()) {
            System.out.println(TEXT_RED + "READ - Cache deu MISS na posição: " + pos + TEXT_RESET);
            syncCacheRam();
        } else {
            System.out.println("READ - Cache deu HIT na posição: " + pos);
        }
    ...
    public void write(int pos, int valor) throws EnderecoInvalido {
        ...
        // Se não estiver no cache, pega da memoria e traz pra cache
        if (isNotOnCache()) {
            System.out.println(TEXT_RED + "WRITE - Cache deu MISS na posição: " + pos + TEXT_RESET);
            syncCacheRam();
        } else {
            System.out.println("WRITE - Cache deu HIT na posição: " + pos);
        }
